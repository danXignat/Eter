...
namespace base {
	//---------------------------------------Constructor-------------------------------------
	MageMode::MageMode(std::vector<ServiceType> services, const std::string& player1_name, const std::string& player2_name)
		: m_board{ 3 },
		m_win_manager{ m_board },
		m_player_red{ player1_name, color::ColorType::RED },
		m_player_blue{ player1_name, color::ColorType::BLUE },
	...
	void MageMode::run() {
		...
		while (m_win_manager.won() == false) {
			...
			if (input.service_type.has_value() && input.service_type == ServiceType::MAGE) {
				m_mage_service.apply(curr_player);
				std::cin.get();
				_switchPlayer();
			}
