...
//    }
//
//    bool Destruction::canUseAbility(const Board& m_board, const Player& player) const {
//        if (!m_board.hasLastPlayedCard()) {
//            return false;
//        }
//        if (m_board.getLastPlayerColor() == player.getColor()) {
//            return false;
//        }
//        return true;
//    }
//
//    std::optional<Coord> Destruction::getTargetPosition(const Board& m_board) const {
//        if (!m_board.hasLastPlayedCard()) {
//            return std::nullopt;
//        }
...
//    }
//
//    std::string Destruction::getErrorMessage(const Board& m_board, const Player& m_player_red) const {
//        if (!m_board.hasLastPlayedCard()) {
//            return "No last played card to destroy";
//        }
//        if (m_board.getLastPlayerColor() == m_player_red.getColor()) {
...
//        if (!canUseAbility(m_board, m_player_red)) {
//            Logger::log(Level::WARNING, getErrorMessage(m_board, m_player_red));
//            return;
//        }
//
...
//
//    ///------------------------------------------ Flame -------------------------------------------
//    Flame::Flame(Board& m_board, Player& red, Player& blue) {                               ///////////////// asta e cu iluzie, nu merge
...
//
//
//    Fire::Fire(Board& m_board, Player& red, Player& blue) {
//        m_ability = PowerCardType::Fire;
//    }
//
//    std::vector<std::pair<Coord, CombatCardType>> Fire::getVisibleCards(const Board& m_board) const {
...
//    }
//
//    bool Fire::canUseAbility(const Board& m_board) const {
//        auto visible_cards = getVisibleCards(m_board);
//        if (visible_cards.empty()) {
//            return false;
//        }
//
//        auto valid_types = getValidChoices(m_board);
//        return !valid_types.empty();
//    }
//
//    std::vector<CombatCardType> Fire::getValidChoices(const Board& m_board) const {
//        auto visible_cards = getVisibleCards(m_board);
...
//    }
//
//    std::string Fire::getErrorMessage(const Board& m_board) const {
//        auto visible_cards = getVisibleCards(m_board);
//        if (visible_cards.empty()) {
...
//    }
//
//    void Fire::applyEffect(Board& m_board, CombatCardType chosen_type) {
//        auto visible_cards = getVisibleCards(m_board);
//
...
//
//    void Fire::apply() {
//        if (!canUseAbility(m_board)) {
//            Logger::log(Level::WARNING, getErrorMessage(m_board));
//            return;
//        }
//
//        auto valid_choices = getValidChoices(m_board);
//
//        if (!m_has_choice) {
...
//        }
//
//        applyEffect(m_board, m_chosen_card);
//        m_has_choice = false;
//    }
//    ////------------------------------------------ Ash -------------------------------------------
//    Ash::Ash(Board& m_board, Player& red, Player& blue) {
...
//
//    ////------------------------------------------ Spark -------------------------------------------
//    Spark::Spark(Board& m_board, Player& red, Player& blue) {
...
//    }
//
//    void Spark::apply(Board& m_board, Player& red, Player& blue) {
//        auto choices = coverCards(m_board, m_player_red);
...
//    }
//
//    std::vector<std::pair<Coord, CombatCardType>> Spark::coverCards(const Board& m_board, const Player& player) {
...
//
//    ////------------------------------------------ Squall -------------------------------------------
//    Squall::Squall(Board& m_board, Player& red, Player& blue) {
...
//    }
//
//    std::vector<std::pair<Coord, CombatCardType>> Squall::getVisibleCards(const Board& m_board, const Player& player) {
...
//    }
//
//    void Squall::apply(Board& m_board, Player& red, Player& blue) {
//        auto visible_cards = getVisibleCards(m_board, m_player_red);
...
//
//    ////------------------------------------------ Gale -------------------------------------------
//    Gale::Gale(Board& m_board, Player& red, Player& blue) {
...
//            m_board.removeCardFromStackAt(coord, CombatCard(card_type, m_player_red.getColor()));
...
//
//    ////------------------------------------------ Hurricane -------------------------------------------
//    Hurricane::Hurricane(Board& m_board, Player& red, Player& blue) {
...
//
//    ////------------------------------------------ Gust -------------------------------------------
//    Gust::Gust(Board& m_board, Player& red, Player& blue) {
...
//    }
//    ////------------------------------------------ Mirrage -------------------------------------------
//    Mirrage::Mirrage(Board& m_board, Player& red, Player& blue) {                                 /////////////// iluzie, nu merge
...
//
//    ////------------------------------------------ Storm -------------------------------------------
//    Storm::Storm(Board& m_board, Player& red, Player& blue) {
...
//
//    ////------------------------------------------ Tide -------------------------------------------
//    Tide::Tide(Board& m_board, Player& red, Player& blue) {
...
//    }
//
//    std::vector<Coord> Tide::getStacks(const Board& m_board) {
...
//
//    void Tide::apply() {
//        std::vector<Coord> stacks = getStacks(m_board);
...
//
//    ////------------------------------------------ Mist -------------------------------------------
//    Mist::Mist(Board& m_board, Player& red, Player& blue) { /////////////////////////////////////////////////////////joaca inca o iluzie, nu merge
...
//
//    ////------------------------------------------ Wave -------------------------------------------
//    Wave::Wave(Board& m_board, Player& red, Player& blue) {
...
//    }
//
//    std::vector<Coord> Wave::validStacks(const Board& m_board) const {
...
//        }
//
//        std::vector<Coord> stacks = validStacks(m_board);
//        setAvailableStacks(stacks);
//
//        if (stacks.empty()) {
...
//
//    ////------------------------------------------ Whirlpool -------------------------------------------
//    Whirlpool::Whirlpool(Board& m_board, Player& red, Player& blue) {
//        m_ability = PowerCardType::Whirlpool;
//    }
//
//    void Whirlpool::apply() {
//        validPairsOnRow = getPairsOnRow(m_board);
//        validPairsOnColumn = getPairsOnColumn(m_board);
//
...
//
//
//    std::vector<std::pair<Coord, Coord>> Whirlpool::getPairsOnRow(Board& m_board) {
...
//    }
//    std::vector<std::pair<Coord, Coord>> Whirlpool::getPairsOnColumn(Board& m_board) {
...
//
//    ////------------------------------------------ Blizzard -------------------------------------------
//    Blizzard::Blizzard(Board& m_board, Player& red, Player& blue) {
...
//            }
//
//            m_board.blockRow(chosen_row, m_player_red.getColor());
...
//            }
//
//            m_board.blockColumn(chosen_column, m_player_red.getColor());
...
//    }
//    ////------------------------------------------ Waterfall -------------------------------------------
//    Waterfall::Waterfall(Board& m_board, Player& red, Player& blue) {
...
//    }
//
//    std::unordered_map<Orientation, std::vector<int16_t>> Waterfall::getOptions(Board& m_board) {
...
//    }
//
//    std::tuple<Orientation, int16_t, MoveDirection> Waterfall::input(Board& m_board) {
//        auto options = getOptions(m_board);
//
//        if (options.empty()) {
//            Logger::log(Level::WARNING, "You can't use this mage right now");
...
//
//    ////------------------------------------------ Support -------------------------------------------
//    Support::Support(Board& m_board, Player& red, Player& blue) {
...
//
//    ////------------------------------------------ Earthquake -------------------------------------------
//    Earthquake::Earthquake(Board& m_board, Player& red, Player& blue) {
...
//
//    ////------------------------------------------ Crumble -------------------------------------------
//    Crumble::Crumble(Board& m_board, Player& red, Player& blue) {
...
//
//
//    Border::Border(Board& m_board, Player& red, Player& blue) {
...
//    ////------------------------------------------ Avalanche -------------------------------------------
//
//    Avalanche::Avalanche(Board& m_board, Player& red, Player& blue) {
...
//    }
//
//    std::vector<std::pair<Orientation, std::pair<Coord, Coord>>> Avalanche::getPairs(Board& m_board) {
...
//        const std::vector<std::pair<Orientation, std::pair<Coord, Coord>>>& pack, Board& m_board) {
//
...
//        auto pairs = getPairs(m_board);
//        if (pairs.empty()) {
//            Logger::log(Level::WARNING, "You can't use this card right now");
//            return;
//        }
//
//        auto choices = checkShifting(pairs, m_board);
//        if (choices.empty()) {
...
//
//    ////------------------------------------------ Rock -------------------------------------------
//    Rock::Rock(Board& m_board, Player& red, Player& blue) {  ///////////////////////////////// foloseste iluzie, nu merge 
